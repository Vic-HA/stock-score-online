...
function getSourceConnectorPlan(config) {
  return [
    ...
    { name: "GoogleFinance / Google Sheet", role: "驗證來源", status: config.googleCsvUrl ? "可測試" : "待填公開 CSV URL", fields: "price、prevClose、volume、PER、EPS", method: "只做 FinMind / TWSE 主資料偏差驗證，不覆蓋總覽主資料。" },
    ...
    <Card className="rounded-xl shadow-sm">
      <CardContent className="p-3 space-y-2">
        ...
        <div className="grid gap-2 md:grid-cols-5">
          ...
          <select className="h-10 rounded-md border border-input bg-background px-2 py-2 text-sm" value={value.type} onChange={(e) => onChange({ ...value, type: e.target.value })}>
            <option value="股票">股票</option>
            <option value="ETF">ETF</option>
          </select>
          <select className="h-10 rounded-md border border-input bg-background px-2 py-2 text-sm" value={value.market} onChange={(e) => onChange({ ...value, market: e.target.value })}>
...
export default function StockShortV1App() {
  ...
  function addAsset() {
    const symbol = String(newAsset.symbol || "").trim().toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 text-slate-900"><div className="mx-auto max-w-7xl space-y-3">
      ...
      <Tabs defaultValue="short" className="space-y-3"><TabsList className="grid w-full grid-cols-6 rounded-xl h-9"><TabsTrigger value="short">短線V1</TabsTrigger><TabsTrigger value="mid">中線V1</TabsTrigger><TabsTrigger value="long">長線V1</TabsTrigger><TabsTrigger value="sources">資料源</TabsTrigger><TabsTrigger value="validate">資料驗證</TabsTrigger><TabsTrigger value="tests">測試</TabsTrigger></TabsList>
